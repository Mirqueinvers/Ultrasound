...
export interface LeftKidneyProtocol {
  length: string;
  width: string;
  thickness: string;
  echogenicity: string;
...
interface LeftKidneyProps {
  value?: LeftKidneyProtocol;
  onChange?: (value: LeftKidneyProtocol) => void;
}

const LeftKidney: React.FC<LeftKidneyProps> = ({ value, onChange }) => {
  const handleChange = (field: keyof LeftKidneyProtocol, newValue: string) => {
    if (onChange) {
      onChange({
        ...value!,
        [field]: newValue,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="md:col-span-3">
          ...
          <textarea
            ...
            className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            rows={2}
            placeholder="Заключение по левой почке..."
          />
        </div>
      </div>
    </div>
