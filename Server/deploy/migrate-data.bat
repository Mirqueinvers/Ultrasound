...
setlocal
chcp 65001 >nul
title Ultrasound - миграция данных (SQLite -> PostgreSQL)

echo ============================================================
echo   Ultrasound - перенос данных Desktop в PostgreSQL
echo   Файл данных: c:\ultrasound\ultrasound.db
echo ============================================================
echo.
...
rem ===== Файл с данными =====
set "DB_SRC=c:\ultrasound\ultrasound.db"
if not exist "%DB_SRC%" (
  echo [ОШИБКА] Не найден файл %DB_SRC%
  echo          Положите ultrasound.db в папку c:\ultrasound и запустите снова.
  pause
  exit /b 1
)
...
set "BACKUP=c:\ultrasound\ultrasound.db.backup_%TS%"
...
rem ===== Миграция =====
echo.
echo Запускаю миграцию (запускается ОДИН раз!)...
cd /d "%SERVER_ROOT%"
call npx tsx prisma/migrate-local-dbs.ts --from "%DB_SRC%"
if errorlevel 1 (
  echo.
  echo [ОШИБКА] Миграция не удалась. Смотрите вывод выше.
  echo          ВАЖНО: не запускайте скрипт повторно до выяснения причины,
  echo          иначе появятся дубли. При необходимости очистите таблицы
  echo          (см. INSTALL.md) и запустите заново.
  pause
  exit /b 1
)
