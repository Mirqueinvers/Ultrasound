...
*
* Этап 3.3/3.4 плана перехода на PostgreSQL: компоненты React обращаются
* к центральному API через IPC. Собственный HTTP-сервер Registry удалён
* (этап 3.4). Обработчики делегируют запросы слою данных `src/db.ts`,
...
//#endregion
//#region electron/autoUpdater.ts
...
function setAutoUpdaterWindow(win) {
	mainWindow$1 = win;
}
function initAutoUpdater() {
	...
	import_main.autoUpdater.on("update-available", (info) => {
		...
		if (mainWindow$1 && !mainWindow$1.isDestroyed()) mainWindow$1.webContents.send("update:available", { version: info.version });
	});
	import_main.autoUpdater.on("update-not-available", (info) => {
		console.log("[AutoUpdater] Update not available");
		if (mainWindow$1 && !mainWindow$1.isDestroyed()) mainWindow$1.webContents.send("update:not-available", { version: info.version });
	});
	import_main.autoUpdater.on("download-progress", (progress) => {
		if (mainWindow$1 && !mainWindow$1.isDestroyed()) mainWindow$1.webContents.send("update:download-progress", {
	...
	import_main.autoUpdater.on("update-downloaded", (info) => {
		...
		if (mainWindow$1 && !mainWindow$1.isDestroyed()) mainWindow$1.webContents.send("update:downloaded", { version: info.version });
	});
	import_main.autoUpdater.on("error", (error) => {
		console.error("[AutoUpdater] Error:", error.message);
		if (mainWindow$1 && !mainWindow$1.isDestroyed()) mainWindow$1.webContents.send("update:error", { message: error.message });
	});
}
function setUpdateServer(ip) {
	const serverUrl = `http://${ip}:8080/registry-updates`;
	console.log("[AutoUpdater] Set update server:", serverUrl);
	import_main.autoUpdater.setFeedURL({
		provider: "generic",
		url: serverUrl
	});
}
function checkForUpdates() {
	import_main.autoUpdater.checkForUpdates().catch((err) => {
		console.error("[AutoUpdater] Check error:", err);
		if (mainWindow$1 && !mainWindow$1.isDestroyed()) mainWindow$1.webContents.send("update:error", { message: err.message });
	});
}
function downloadUpdate() {
	import_main.autoUpdater.downloadUpdate().catch((err) => {
...
electron.app.whenReady().then(async () => {
	...
	function normalizeServers(parsed) {
		return (Array.isArray(parsed) ? parsed : parsed && typeof parsed === "object" && Array.isArray(parsed.servers) ? parsed.servers : []).map((entry) => {
			...
			if (entry && typeof entry === "object" && typeof entry.ip === "string") {
				const item = entry;
				return {
					name: typeof item.name === "string" && item.name.trim() !== "" ? item.name : item.ip,
					ip: item.ip
				};
			}
			return null;
	...
	electron.ipcMain.handle("update:getServers", async () => {
		try {
			const data = await node_fs.promises.readFile(updateServersFilePath, "utf8");
			return normalizeServers(JSON.parse(data));
		} catch {
			return [];
		}
	});
	electron.ipcMain.handle("update:saveServers", async (_, servers) => {
		try {
			const existing = await node_fs.promises.readFile(updateServersFilePath, "utf8").then((d) => JSON.parse(d), () => ({}));
			const merged = {
				servers,
				activeIp: typeof existing.activeIp === "string" ? existing.activeIp : ""
			};
			await node_fs.promises.writeFile(updateServersFilePath, JSON.stringify(merged, null, 2), "utf8");
			return { success: true };
		} catch (error) {
			console.error("Update servers save error:", error);
			return {
				success: false,
				message: "Не удалось сохранить серверы обновлений"
			};
		}
	});
	electron.ipcMain.handle("update:getActiveServer", async () => {
		try {
			const data = await node_fs.promises.readFile(updateServersFilePath, "utf8");
			const parsed = JSON.parse(data);
			if (parsed && typeof parsed === "object" && typeof parsed.activeIp === "string") return parsed.activeIp;
			return "";
		} catch {
			return "";
		}
	});
	electron.ipcMain.handle("update:setActiveServer", async (_, ip) => {
		try {
			const merged = {
				servers: normalizeServers((await node_fs.promises.readFile(updateServersFilePath, "utf8").then((d) => JSON.parse(d), () => ({}))).servers),
				activeIp: ip
			};
			await node_fs.promises.writeFile(updateServersFilePath, JSON.stringify(merged, null, 2), "utf8");
			return { success: true };
			...
			return {
				success: false,
				message: "Не удалось сохранить активный сервер"
			};
		}
	});
	electron.ipcMain.handle("update:check", async () => {
		try {
			const data = await node_fs.promises.readFile(updateServersFilePath, "utf8");
			const parsed = JSON.parse(data);
			if (parsed && typeof parsed === "object" && typeof parsed.activeIp === "string" && parsed.activeIp.trim() !== "") setUpdateServer(parsed.activeIp.trim());
		} catch {}
		checkForUpdates();
	});
	electron.ipcMain.handle("update:download", () => {
		downloadUpdate();
	});
